...
- **[TMDB 助手](./tmdb-helper/)**（[Greasy Fork 主页](https://greasyfork.org/zh-CN/scripts/594907-tmdb-%E5%8A%A9%E6%89%8B)）：themoviedb.org 页面助手——多数据源搜索、资料一键复制、悬浮球右键复制 `{tmdbid=…}` 标记；季编辑页批量单集、图片直传、剧集组管理（只查资料，不回写页面）。适用 themoviedb.org。
