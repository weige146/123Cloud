...
- 当前线上版本：`1.3.5`（核对于 2026-09-11；本地开发随即从 `1.3.6` 起步，1.3.6 内容：整理预览季集编号高亮由荧光黄底色改为红色字体）。**`1.3.0` 已随仓库推送**（维护者 2026-09-08 明确指示跨过「只 +1 补丁号」规则直接升 1.3.0，作为例外记录在案），1.3.0 内容：fileInfos 33 条截断补查、分享链接生成秒传的风控实测修复（旧域名直连 123865 + 分车道限速门 + 32 目录并发）、秒传导出/导入并发按接口实测对齐（8/32）+ 100011 频控退避重试、TMDB 校准与媒体整理增强、选中项出口统一按文件名自然排序。
